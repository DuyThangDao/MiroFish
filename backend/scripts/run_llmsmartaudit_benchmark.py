...
For each GT contract in a contest:
  ...
  4. Save to benchmark/web3bugs/llmsmartaudit/<contest_id>/

Usage:
    cd /home/thangdd/repos/MiroFish/backend
    source .venv/bin/activate
    python scripts/run_llmsmartaudit_benchmark.py --contest-id 104
    python scripts/run_llmsmartaudit_benchmark.py --all

Output:
    benchmark/web3bugs/llmsmartaudit/<id>/findings_<Contract>.json   (per-contract)
    benchmark/web3bugs/llmsmartaudit/<id>/findings_all.json          (aggregated)

Requirements:
    LLM5_VERTEX_AI_KEY_FILE and LLM5_BASE_URL in .env (MiroFish root)
"""
...
OUT_ROOT = REPO_ROOT / "benchmark/web3bugs/llmsmartaudit"
...
def build_env() -> dict:
    ...
    # Map LLM5_* → VERTEX_AI_KEY_FILE / LLM5_BASE_URL (model_backend reads both)
...
def run_one_contract(
    ...
    print(f"    CMD: {' '.join(cmd[:6])} ... --name {run_name}")
    print(f"    Source: {sol_file}")
    ...
    # Find WareHouse log: <run_name>_Benchmark_<timestamp>.log
    logs = sorted(WAREHOUSE_DIR.glob(f"{run_name}_Benchmark_*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def process_contest(contest_id: str) -> dict:
    ...
    for contract_name in cfg["gt_contracts"]:
        print(f"\n  [{contract_name}]")
        ...
        if not sol_file:
            ...
            contract_results.append({
                "contract_name": contract_name,
                "status": "sol_not_found",
                "findings": [],
            })
            continue

        # Unique run name to avoid WareHouse collisions
        ...
        t0 = time.time()
        ok, wh_log = run_one_contract(contract_name, sol_file, run_name, env, console_log)
        ...
        # Copy WareHouse log to our output dir for reference
        ...
        print(f"    Done in {elapsed:.0f}s — {len(findings)} findings extracted")

        # Save per-contract JSON
        per_contract = {
...
def main():
    ...
    missing = [k for k in ("LLM5_VERTEX_AI_KEY_FILE", "LLM5_BASE_URL") if k not in ENV_VARS]
    if missing:
