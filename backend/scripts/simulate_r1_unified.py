"""
Simulation với source code THẬT từ contest 42 — dùng solodit_unified RAG.
So sánh no_rag vs with_rag (solodit_unified: structured vul+inv+op, avg 1256 chars).
Baseline: simulate_r1_real.py dùng solodit_findings (raw blob, avg 4911 chars).
"""
...
_col = _chroma.get_collection('solodit_unified', embedding_function=_embed)
print(f"[RAG] solodit_unified: {_col.count()} docs loaded")
...
RAG_TOOL = [{
    ...
    "function": {
        ...
        "description": (
            ...
            "Each result contains: vulnerability description, violated invariant, and operations involved. "
            "Call when you suspect a vulnerability pattern — recall how similar bugs appeared in past protocols. "
            "Query with the specific mechanism (e.g. 'storage mapping overwrite no existence check', "
            "'global debt tracker inconsistent with individual positions', 'swap no slippage protection'). "
        ...
        "parameters": {
            ...
            "properties": {
                "query": {"type": "string",
                          "description": "Describe the specific vulnerability pattern or invariant violation you suspect."}
            },
            "required": ["query"]
        }
    }
...
def search_rag(query: str) -> str:
    ...
    for doc, meta, dist in zip(docs, metas, dists):
        ...
        firm = meta.get('firm','')
        # solodit_unified docs are already structured (avg 1256 chars) — show full
...
def make_prompt(agent_id, contract, source, with_rag: bool):
    ...
    if with_rag:
        rag_block = (
            ...
            "RULE 2: Each query must state a specific mechanism hypothesis.\n"
            "  Format: '[what breaks mechanically] + [specific variable or code detail]'\n"
...
def main():
    out_dir = "/home/thangdd/repos/MiroFish/benchmark/web3bugs/agent-redesign/42/sim_unified_hyp_cross"
