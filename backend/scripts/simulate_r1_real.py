"""
Simulation với source code THẬT từ contest 42.
Không có hints, không có BUG comments.
Ghi response đầy đủ ra file để manual review.
"""
...
RAG_TOOL = [{
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "query": {"type": "string",
                          "description": "Describe the specific vulnerability pattern you suspect."}
            },
            "required": ["query"]
        }
    }
    ...
    for _ in range(6):  # max turns
        ...
        if msg.tool_calls and with_rag:
            for tc in msg.tool_calls:
                ...
                q = args.get("query", "")
                print(f"      RAG[{rag_n+1}]: '{q[:70]}'", flush=True)
                result = search_rag(q)
                rag_n += 1
...
def make_prompt(agent_id, contract, source, with_rag: bool):
    ...
    if with_rag:
        rag_block = (
            ...
            "  'global accumulator inconsistent individual positions'\n"
            "  'swap no slippage protection sandwich attack'\n"
            "  'reward balance not reset before external call'\n"
...
def main():
    out_dir = "/home/thangdd/repos/MiroFish/benchmark/web3bugs/agent-redesign/42/sim_real"
