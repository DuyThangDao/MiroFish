...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # this is required by the Zep SDK; warnings come from dynamic class creation and are safe to ignore
        ...
        def safe_attr_name(attr_name: str) -> str:
            """Convert a reserved name to a safe alternative."""
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to finish processing (polls each episode's processed status)."""
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Some episodes timed out — completed {completed_count}/{total_episodes}",
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch full graph data (including detailed node/edge info).

        Args:
            graph_id: graph ID

        Returns:
            dict with nodes and edges lists, including timestamps, attributes, and other details
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # build node lookup for resolving node names on edges
        node_map = {}
        for node in nodes:
